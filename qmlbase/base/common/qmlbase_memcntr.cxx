#ifndef QMLBASE_MEMCNTR_CXX
#define QMLBASE_MEMCNTR_CXX

#include "qmlbase_memcntr.hxx"
#include "qmlbase_logging.hxx"
#include "qmlbase_semtemp.hpp"
#include "qmlbase_dequetemp.hpp"
#include "qmlbase_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLBASE_HIDDEN MemCntrRec {
...
class QMLBASE_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLBASE_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLBASE_HIDDEN MemCntrRecAction {
...
typedef qmlbase::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlbase::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLBASE_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlbase_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlbase_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlbase_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
#else
    QMLBASE_UNUSED(file); QMLBASE_UNUSED(func);
    QMLBASE_UNUSED(ln);   QMLBASE_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlbase_fatal("allocated memory, but return nullptr!");
        } else {
            qmlbase_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlbase_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlbase_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

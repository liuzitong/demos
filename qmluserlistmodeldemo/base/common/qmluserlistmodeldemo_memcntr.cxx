#ifndef QMLUSERLISTMODELDEMO_MEMCNTR_CXX
#define QMLUSERLISTMODELDEMO_MEMCNTR_CXX

#include "qmluserlistmodeldemo_memcntr.hxx"
#include "qmluserlistmodeldemo_logging.hxx"
#include "qmluserlistmodeldemo_semtemp.hpp"
#include "qmluserlistmodeldemo_dequetemp.hpp"
#include "qmluserlistmodeldemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLUSERLISTMODELDEMO_HIDDEN MemCntrRec {
...
class QMLUSERLISTMODELDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLUSERLISTMODELDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLUSERLISTMODELDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlUserListmodelDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlUserListmodelDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLUSERLISTMODELDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmluserlistmodeldemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmluserlistmodeldemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmluserlistmodeldemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLUSERLISTMODELDEMO_UNUSED(func);
        QMLUSERLISTMODELDEMO_UNUSED(ln);
    }
#else
    QMLUSERLISTMODELDEMO_UNUSED(file); QMLUSERLISTMODELDEMO_UNUSED(func);
    QMLUSERLISTMODELDEMO_UNUSED(ln);   QMLUSERLISTMODELDEMO_UNUSED(type);
#endif
    return ct;
}
...
#else
    QMLUSERLISTMODELDEMO_UNUSED( fn );
#endif
...
void MemCntr :: enableMemTrace( bool sw )
{
    ...
    gEnableMemTrace( sw );
#else
    QMLUSERLISTMODELDEMO_UNUSED( sw );
#endif
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmluserlistmodeldemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmluserlistmodeldemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmluserlistmodeldemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmluserlistmodeldemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

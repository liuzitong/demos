#ifndef QMLMSGBUSDEMO_MEMCNTR_CXX
#define QMLMSGBUSDEMO_MEMCNTR_CXX

#include "qmlmsgbusdemo_memcntr.hxx"
#include "qmlmsgbusdemo_logging.hxx"
#include "qmlmsgbusdemo_semtemp.hpp"
#include "qmlmsgbusdemo_dequetemp.hpp"
#include "qmlmsgbusdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLMSGBUSDEMO_HIDDEN MemCntrRec {
...
class QMLMSGBUSDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLMSGBUSDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLMSGBUSDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlMsgbusDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlMsgbusDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLMSGBUSDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlmsgbusdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlmsgbusdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlmsgbusdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLMSGBUSDEMO_UNUSED(func);
        QMLMSGBUSDEMO_UNUSED(ln);
    }
#else
    QMLMSGBUSDEMO_UNUSED(file); QMLMSGBUSDEMO_UNUSED(func);
    QMLMSGBUSDEMO_UNUSED(ln);   QMLMSGBUSDEMO_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlmsgbusdemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmlmsgbusdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlmsgbusdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlmsgbusdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

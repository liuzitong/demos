#ifndef CONFIRMDEMO_MEMCNTR_CXX
#define CONFIRMDEMO_MEMCNTR_CXX

#include "confirmdemo_memcntr.hxx"
#include "confirmdemo_logging.hxx"
#include "confirmdemo_semtemp.hpp"
#include "confirmdemo_dequetemp.hpp"
#include "confirmdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class CONFIRMDEMO_HIDDEN MemCntrRec {
...
class CONFIRMDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class CONFIRMDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class CONFIRMDEMO_HIDDEN MemCntrRecAction {
...
typedef confirmDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef confirmDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class CONFIRMDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  confirmdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = confirmdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *confirmdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        CONFIRMDEMO_UNUSED(func);
        CONFIRMDEMO_UNUSED(ln);
    }
#else
    CONFIRMDEMO_UNUSED(file); CONFIRMDEMO_UNUSED(func);
    CONFIRMDEMO_UNUSED(ln);   CONFIRMDEMO_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            confirmdemo_fatal("allocated memory, but return nullptr!");
        } else {
            confirmdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = confirmdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = confirmdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

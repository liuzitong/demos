#ifndef QMLMSGBUSMULTIWINDEMO_MEMCNTR_CXX
#define QMLMSGBUSMULTIWINDEMO_MEMCNTR_CXX

#include "qmlmsgbusmultiwindemo_memcntr.hxx"
#include "qmlmsgbusmultiwindemo_logging.hxx"
#include "qmlmsgbusmultiwindemo_semtemp.hpp"
#include "qmlmsgbusmultiwindemo_dequetemp.hpp"
#include "qmlmsgbusmultiwindemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLMSGBUSMULTIWINDEMO_HIDDEN MemCntrRec {
...
class QMLMSGBUSMULTIWINDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLMSGBUSMULTIWINDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLMSGBUSMULTIWINDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlMsgbusMultiwinDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlMsgbusMultiwinDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLMSGBUSMULTIWINDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlmsgbusmultiwindemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlmsgbusmultiwindemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlmsgbusmultiwindemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLMSGBUSMULTIWINDEMO_UNUSED(func);
        QMLMSGBUSMULTIWINDEMO_UNUSED(ln);
    }
#else
    QMLMSGBUSMULTIWINDEMO_UNUSED(file); QMLMSGBUSMULTIWINDEMO_UNUSED(func);
    QMLMSGBUSMULTIWINDEMO_UNUSED(ln);   QMLMSGBUSMULTIWINDEMO_UNUSED(type);
#endif
    return ct;
}
...
#else
    QMLMSGBUSMULTIWINDEMO_UNUSED( fn );
#endif
...
void MemCntr :: enableMemTrace( bool sw )
{
    ...
    gEnableMemTrace( sw );
#else
    QMLMSGBUSMULTIWINDEMO_UNUSED( sw );
#endif
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlmsgbusmultiwindemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmlmsgbusmultiwindemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlmsgbusmultiwindemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlmsgbusmultiwindemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

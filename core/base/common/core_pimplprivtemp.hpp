...
*/
/* ///////////////////////////////////////////////////////////////////// */
#ifndef  CORE_PIMPLPRIVTEMP_HXX
#define  CORE_PIMPLPRIVTEMP_HXX
...
#define CORE_PImplPrivTemp_PtrCast( t, o ) (  reinterpret_cast< t >( reinterpret_cast< intptr_t >( o ))  )
#define CORE_PImplPrivTemp_IntPtrCast( o ) (  reinterpret_cast< intptr_t >( o ))


#ifndef CORE_PImplPrivTemp_new
#define CORE_PImplPrivTemp_new( t, ... )  ( new t( __VA_ARGS__ ))
#endif

#ifndef CORE_PImplPrivTemp_delete
#define CORE_PImplPrivTemp_delete( o, t )  do{ delete CORE_PImplPrivTemp_PtrCast( t*,  o ); }while(0)
#endif
...
public:   
    ...
    static T*    createInstance ( )           { return CORE_PImplPrivTemp_new( T ); }
    static T*    createInstance ( void *ref ) { return CORE_PImplPrivTemp_new( T,  * CORE_PImplPrivTemp_PtrCast( T*, ref ) ); }
    static T*    addRef    ( void *cd )       {
        if ( cd != nullptr ) {
            CORE_PImplPrivTemp_PtrCast( T*, cd )->addRefCntr( );
        }
        return  CORE_PImplPrivTemp_PtrCast( T*, cd );
    }
...
#define CORE_PImplPrivTemp_AttachPtr( t, s, d )\
    t :: attach( CORE_PImplPrivTemp_PtrCast( void**, s ), CORE_PImplPrivTemp_PtrCast( void**, d ))

#define CORE_PImplPrivTemp_BuildIfNull( t, d ) \
    t :: buildIfNull( CORE_PImplPrivTemp_PtrCast( void**, d ) )
...
T*   PImplPrivTemp<T> :: buildIfNull( void **w_obj )
{
    ...
    //return ( T *)( *w_obj );
    return CORE_PImplPrivTemp_PtrCast( T*, *w_obj );
}
...
bool  PImplPrivTemp<T> :: releaseRef ( void *cd )
{
    ...
    if ( cd != nullptr ) {
        ...
        if ( CORE_PImplPrivTemp_PtrCast( T*, cd )->releaseRefCntr() == 0 ) {
            CORE_PImplPrivTemp_delete( cd, T );
            is_released = true;
        }
    }   	
    return is_released;
...
T *    PImplPrivTemp<T> :: duplicateIfShared( void **w_obj )
{
    ...
    if ( this->currRefCntr( ) == 1 ) { return CORE_PImplPrivTemp_PtrCast( T *, *w_obj ); }
...
//    if ( (( T *) *w_obj)->currRefCntr( ) == 1 ) { return ( T *)( *w_obj ); }
    if ( CORE_PImplPrivTemp_PtrCast( T*, *w_obj)->currRefCntr( ) == 1 ) {
        return CORE_PImplPrivTemp_PtrCast( T*, *w_obj );
    }

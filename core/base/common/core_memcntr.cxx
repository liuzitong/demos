#ifndef CORE_MEMCNTR_CXX
#define CORE_MEMCNTR_CXX

#include "core_memcntr.hxx"
#include "core_logging.hxx"
#include "core_semtemp.hpp"
#include "core_dequetemp.hpp"
#include "core_dyncinit_priv.hxx"
...
class CORE_HIDDEN MemCntrConstStrCmp {
public:
...
class CORE_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class CORE_HIDDEN MemCntrRecAction {
...
typedef core::Deque<MemCntrRec>        MemCntrRecQueue;
typedef core::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class CORE_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  core_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = core_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *core_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
#else
    CORE_UNUSED(file); CORE_UNUSED(func);
    CORE_UNUSED(ln);   CORE_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            core_fatal("allocated memory, but return nullptr!");
        } else {
            core_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = core_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = core_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

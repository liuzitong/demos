#ifndef QMLICQUICKIMAGEITEMDEMO_MEMCNTR_CXX
#define QMLICQUICKIMAGEITEMDEMO_MEMCNTR_CXX

#include "qmlicquickimageitemdemo_memcntr.hxx"
#include "qmlicquickimageitemdemo_logging.hxx"
#include "qmlicquickimageitemdemo_semtemp.hpp"
#include "qmlicquickimageitemdemo_dequetemp.hpp"
#include "qmlicquickimageitemdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLICQUICKIMAGEITEMDEMO_HIDDEN MemCntrRec {
...
class QMLICQUICKIMAGEITEMDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLICQUICKIMAGEITEMDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLICQUICKIMAGEITEMDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlIcquickimageitemDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlIcquickimageitemDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLICQUICKIMAGEITEMDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlicquickimageitemdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlicquickimageitemdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlicquickimageitemdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLICQUICKIMAGEITEMDEMO_UNUSED(func);
        QMLICQUICKIMAGEITEMDEMO_UNUSED(ln);
    }
#else
    QMLICQUICKIMAGEITEMDEMO_UNUSED(file); QMLICQUICKIMAGEITEMDEMO_UNUSED(func);
    QMLICQUICKIMAGEITEMDEMO_UNUSED(ln);   QMLICQUICKIMAGEITEMDEMO_UNUSED(type);
#endif
    return ct;
}
...
#else
    QMLICQUICKIMAGEITEMDEMO_UNUSED( fn );
#endif
...
void MemCntr :: enableMemTrace( bool sw )
{
    ...
    gEnableMemTrace( sw );
#else
    QMLICQUICKIMAGEITEMDEMO_UNUSED( sw );
#endif
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlicquickimageitemdemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmlicquickimageitemdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlicquickimageitemdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlicquickimageitemdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

#ifndef LIBANDQMLTESTDEMO_MEMCNTR_CXX
#define LIBANDQMLTESTDEMO_MEMCNTR_CXX

#include "libandqmltestdemo_memcntr.hxx"
#include "libandqmltestdemo_logging.hxx"
#include "libandqmltestdemo_semtemp.hpp"
#include "libandqmltestdemo_dequetemp.hpp"
#include "libandqmltestdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class LIBANDQMLTESTDEMO_HIDDEN MemCntrRec {
...
class LIBANDQMLTESTDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class LIBANDQMLTESTDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class LIBANDQMLTESTDEMO_HIDDEN MemCntrRecAction {
...
typedef libandqmltestdemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef libandqmltestdemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class LIBANDQMLTESTDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  libandqmltestdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = libandqmltestdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *libandqmltestdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        LIBANDQMLTESTDEMO_UNUSED(func);
        LIBANDQMLTESTDEMO_UNUSED(ln);
    }
#else
    LIBANDQMLTESTDEMO_UNUSED(file); LIBANDQMLTESTDEMO_UNUSED(func);
    LIBANDQMLTESTDEMO_UNUSED(ln);   LIBANDQMLTESTDEMO_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            libandqmltestdemo_fatal("allocated memory, but return nullptr!");
        } else {
            libandqmltestdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = libandqmltestdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = libandqmltestdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

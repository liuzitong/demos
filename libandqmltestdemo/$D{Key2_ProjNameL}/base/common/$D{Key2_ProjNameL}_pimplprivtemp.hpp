...
*/
/* ///////////////////////////////////////////////////////////////////// */
#ifndef  $D{Key2_ProjNameU}_PIMPLPRIVTEMP_HXX
#define  $D{Key2_ProjNameU}_PIMPLPRIVTEMP_HXX
...
#define $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( t, o ) (  reinterpret_cast< t >( reinterpret_cast< intptr_t >( o ))  )
#define $D{Key2_ProjNameU}_PImplPrivTemp_IntPtrCast( o ) (  reinterpret_cast< intptr_t >( o ))


#ifndef $D{Key2_ProjNameU}_PImplPrivTemp_new
#define $D{Key2_ProjNameU}_PImplPrivTemp_new( t, ... )  ( new t( __VA_ARGS__ ))
#endif

#ifndef $D{Key2_ProjNameU}_PImplPrivTemp_delete
#define $D{Key2_ProjNameU}_PImplPrivTemp_delete( o, t )  do{ delete $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( t*,  o ); }while(0)
#endif
...
public:   
    ...
    static T*    createInstance ( )           { return $D{Key2_ProjNameU}_PImplPrivTemp_new( T ); }
    static T*    createInstance ( void *ref ) { return $D{Key2_ProjNameU}_PImplPrivTemp_new( T,  * $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, ref ) ); }
    static T*    addRef    ( void *cd )       {
        if ( cd != nullptr ) {
            $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, cd )->addRefCntr( );
        }
        return  $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, cd );
    }
...
#define $D{Key2_ProjNameU}_PImplPrivTemp_AttachPtr( t, s, d )\
    t :: attach( $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( void**, s ), $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( void**, d ))

#define $D{Key2_ProjNameU}_PImplPrivTemp_BuildIfNull( t, d ) \
    t :: buildIfNull( $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( void**, d ) )
...
T*   PImplPrivTemp<T> :: buildIfNull( void **w_obj )
{
    ...
    //return ( T *)( *w_obj );
    return $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, *w_obj );
}
...
bool  PImplPrivTemp<T> :: releaseRef ( void *cd )
{
    ...
    if ( cd != nullptr ) {
        ...
        if ( $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, cd )->releaseRefCntr() == 0 ) {
            $D{Key2_ProjNameU}_PImplPrivTemp_delete( cd, T );
            is_released = true;
        }
    }   	
    return is_released;
...
T *    PImplPrivTemp<T> :: duplicateIfShared( void **w_obj )
{
    ...
    if ( this->currRefCntr( ) == 1 ) { return $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T *, *w_obj ); }
...
//    if ( (( T *) *w_obj)->currRefCntr( ) == 1 ) { return ( T *)( *w_obj ); }
    if ( $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, *w_obj)->currRefCntr( ) == 1 ) {
        return $D{Key2_ProjNameU}_PImplPrivTemp_PtrCast( T*, *w_obj );
    }

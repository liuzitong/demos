#ifndef $D{Key2_ProjNameU}_MEMCNTR_CXX
#define $D{Key2_ProjNameU}_MEMCNTR_CXX

#include "$D{Key2_ProjNameL}_memcntr.hxx"
#include "$D{Key2_ProjNameL}_logging.hxx"
#include "$D{Key2_ProjNameL}_semtemp.hpp"
#include "$D{Key2_ProjNameL}_dequetemp.hpp"
#include "$D{Key2_ProjNameL}_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class $D{Key2_ProjNameU}_HIDDEN MemCntrRec {
...
class $D{Key2_ProjNameU}_HIDDEN MemCntrConstStrCmp {
public:
...
class $D{Key2_ProjNameU}_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class $D{Key2_ProjNameU}_HIDDEN MemCntrRecAction {
...
typedef $D{Key2_ProjNameN}::Deque<MemCntrRec>        MemCntrRecQueue;
typedef $D{Key2_ProjNameN}::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class $D{Key2_ProjNameU}_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  $D{Key2_ProjNameL}_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = $D{Key2_ProjNameL}_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *$D{Key2_ProjNameL}_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        $D{Key2_ProjNameU}_UNUSED(func);
        $D{Key2_ProjNameU}_UNUSED(ln);
    }
#else
    $D{Key2_ProjNameU}_UNUSED(file); $D{Key2_ProjNameU}_UNUSED(func);
    $D{Key2_ProjNameU}_UNUSED(ln);   $D{Key2_ProjNameU}_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            $D{Key2_ProjNameL}_fatal("allocated memory, but return nullptr!");
        } else {
            $D{Key2_ProjNameL}_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = $D{Key2_ProjNameL}_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = $D{Key2_ProjNameL}_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

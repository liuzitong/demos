#ifndef QMLCONFIRMBUSDEMO_MEMCNTR_CXX
#define QMLCONFIRMBUSDEMO_MEMCNTR_CXX

#include "qmlconfirmbusdemo_memcntr.hxx"
#include "qmlconfirmbusdemo_logging.hxx"
#include "qmlconfirmbusdemo_semtemp.hpp"
#include "qmlconfirmbusdemo_dequetemp.hpp"
#include "qmlconfirmbusdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLCONFIRMBUSDEMO_HIDDEN MemCntrRec {
...
class QMLCONFIRMBUSDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLCONFIRMBUSDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLCONFIRMBUSDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlconfirmbusdemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlconfirmbusdemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLCONFIRMBUSDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlconfirmbusdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlconfirmbusdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlconfirmbusdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLCONFIRMBUSDEMO_UNUSED(func);
        QMLCONFIRMBUSDEMO_UNUSED(ln);
    }
#else
    QMLCONFIRMBUSDEMO_UNUSED(file); QMLCONFIRMBUSDEMO_UNUSED(func);
    QMLCONFIRMBUSDEMO_UNUSED(ln);   QMLCONFIRMBUSDEMO_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlconfirmbusdemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmlconfirmbusdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlconfirmbusdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlconfirmbusdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

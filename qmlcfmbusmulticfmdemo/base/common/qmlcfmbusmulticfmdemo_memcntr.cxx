#ifndef QMLCFMBUSMULTICFMDEMO_MEMCNTR_CXX
#define QMLCFMBUSMULTICFMDEMO_MEMCNTR_CXX

#include "qmlcfmbusmulticfmdemo_memcntr.hxx"
#include "qmlcfmbusmulticfmdemo_logging.hxx"
#include "qmlcfmbusmulticfmdemo_semtemp.hpp"
#include "qmlcfmbusmulticfmdemo_dequetemp.hpp"
#include "qmlcfmbusmulticfmdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLCFMBUSMULTICFMDEMO_HIDDEN MemCntrRec {
...
class QMLCFMBUSMULTICFMDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLCFMBUSMULTICFMDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLCFMBUSMULTICFMDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlCfmBusMulticfmDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlCfmBusMulticfmDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLCFMBUSMULTICFMDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlcfmbusmulticfmdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlcfmbusmulticfmdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlcfmbusmulticfmdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLCFMBUSMULTICFMDEMO_UNUSED(func);
        QMLCFMBUSMULTICFMDEMO_UNUSED(ln);
    }
#else
    QMLCFMBUSMULTICFMDEMO_UNUSED(file); QMLCFMBUSMULTICFMDEMO_UNUSED(func);
    QMLCFMBUSMULTICFMDEMO_UNUSED(ln);   QMLCFMBUSMULTICFMDEMO_UNUSED(type);
#endif
    return ct;
}
...
#else
    QMLCFMBUSMULTICFMDEMO_UNUSED( fn );
#endif
...
void MemCntr :: enableMemTrace( bool sw )
{
    ...
    gEnableMemTrace( sw );
#else
    QMLCFMBUSMULTICFMDEMO_UNUSED( sw );
#endif
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlcfmbusmulticfmdemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmlcfmbusmulticfmdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlcfmbusmulticfmdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlcfmbusmulticfmdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;

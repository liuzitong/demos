...
protected:
    static int  texFmtFromPxFmt   ( int px_fmt ){ return ( px_fmt < sizeof( gltex_fmt )   / sizeof( gltex_fmt[0]    ) ? gltex_fmt[ px_fmt ]    : GL_NONE ); }
    static bool hasAlphaFromPxFmt ( int px_fmt ){ return ( px_fmt < sizeof( has_alpha_ch )/ sizeof( has_alpha_ch[0] ) ? has_alpha_ch[ px_fmt ] : false  ); }
    static int  fmtBytesFromPxFmt ( int px_fmt ){ return ( px_fmt < sizeof( fmt_bytes )   / sizeof( fmt_bytes[0]    ) ? fmt_bytes[ px_fmt ]    : 1 ); }
    static int  qim2glFmtFromPxFmt( int px_fmt ){ return ( px_fmt < sizeof( qim2gl_fmt )  / sizeof( qim2gl_fmt[0]   ) ? qim2gl_fmt[ px_fmt ]   : GL_RGBA ); }
    static int  qim2glBtFromPxFmt ( int px_fmt ){ return ( px_fmt < sizeof( qim2gl_bt  )  / sizeof( qim2gl_bt[0]    ) ? qim2gl_bt[ px_fmt ]    : GL_UNSIGNED_BYTE ); }
public :
    IcQSGImTexturePriv(  );
...
// ctor by another
// ===============================================================
        IcQSGImTexture :: IcQSGImTexture ( const IcQSGImTexture &other )
{
    m_obj = nullptr;
...
// ===============================================================
bool    IcQSGImTexture :: isNull( ) const { return ( m_obj == Q_NULLPTR ); }

// ===============================================================
//! override functions, see QSGTexture
// ===============================================================
void   IcQSGImTexture :: bind ( ) { if ( m_obj != Q_NULLPTR ) { T_PrivPtr( m_obj )->bind(); } }
bool   IcQSGImTexture :: hasAlphaChannel ( ) const { return ( m_obj != Q_NULLPTR ? T_PrivPtr( m_obj )->hasAlpha()  : false ); }
bool   IcQSGImTexture :: hasMipmaps ( )      const { return false; }
int    IcQSGImTexture :: textureId ( )       const { return ( m_obj != Q_NULLPTR ? T_PrivPtr( m_obj )->textureId() : 0 ); }
QSize  IcQSGImTexture :: textureSize ( )     const { return ( m_obj != Q_NULLPTR ? T_PrivPtr( m_obj )->texSize()   : QSize()); }

bool   IcQSGImTexture :: updateTexture ( const void *bits, int width, int height, QImage::Format fmt )
{
    return ( m_obj != Q_NULLPTR ? T_PrivPtr( m_obj )->updateTexture( bits, width, height, fmt ) : false);
}

bool   IcQSGImTexture :: updateTexture ( const QImage & im )
{
    if ( im.isNull() || m_obj == Q_NULLPTR ) { return false; }
...
// ===============================================================
// return the maximum texture size  [ static ]
// ===============================================================

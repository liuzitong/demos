...
void   Test :: create_NullObject()
{
    ...
    bool is_pt_null = false; bool is_pt_d_null = false;
    bool is_pt_f_null = false;
    {
        QxPack::IcPoint  pt;
        QxPack::IcPointD pt_d;
        QxPack::IcPointF pt_f;
        is_pt_null   = pt.isNull();
        is_pt_f_null = pt_f.isNull();
        is_pt_d_null = pt_d.isNull();
    }
    ...
    QVERIFY2( is_pt_d_null, "IcPointD is not null." );
}
...
void   Test :: create_SpecPoint()
{
    ...
    float  xf = 10, yf = 10;
    double xd = 10, yd = 10;
    bool   is_pt_ok, is_pt_f_ok, is_pt_d_ok;
    {
        QxPack::IcPoint  pt  ( x, y );
        QxPack::IcPointD pt_d( xf, yf );
        QxPack::IcPointF pt_f( xd, yd );
        is_pt_ok   = ( pt.x() == x && pt.y() == y );
        is_pt_f_ok = ( QxPack::fuzzyCompare( pt_f.x(), xf )  &&  QxPack::fuzzyCompare( pt_f.y(), yf ));
        is_pt_d_ok = ( QxPack::fuzzyCompare( pt_d.x(), xd )  &&  QxPack::fuzzyCompare( pt_d.y(), yd ));
    }
    int curr_cntr = QxPack::IcMemCntr::currNewCntr();
    QVERIFY2( mem_cntr == curr_cntr, "memory leak!");
    QVERIFY2( is_pt_ok, "IcPoint  do not point to (10,10)." );
    QVERIFY2( is_pt_ok, "IcPointF do not point to (10.0,10.0)." );
    QVERIFY2( is_pt_ok, "IcPointD do not point to (10.0,10.0)." );
}
...
void    Test :: create_ByAnother()
{
    ...
    bool is_pt_same, is_pt_d_same, is_pt_f_same;
    {
        ...
        QxPack::IcPointD pt_d1( 10, 10 );  QxPack::IcPointD pt_d2( pt_d1 );
        ...
        is_pt_d_same = (
            QxPack::fuzzyCompare( pt_d1.x(), pt_d2.x()) &&
            QxPack::fuzzyCompare( pt_d1.y(), pt_d2.y())
        );
    }
    int curr_cntr = QxPack::IcMemCntr::currNewCntr();
    QVERIFY2( mem_cntr == curr_cntr, "memory leak!");
    ...
    QVERIFY2( is_pt_d_same, "IcPointD create by Another is not same." );
}
...
void    Test :: create_ByAssign()
{
    ...
    QVERIFY2( is_pt_d_same, "IcPointD create by assign is not same." );
}

#ifndef QMLPAGELAYOUTDEMO_MEMCNTR_CXX
#define QMLPAGELAYOUTDEMO_MEMCNTR_CXX

#include "qmlpagelayoutdemo_memcntr.hxx"
#include "qmlpagelayoutdemo_logging.hxx"
#include "qmlpagelayoutdemo_semtemp.hpp"
#include "qmlpagelayoutdemo_dequetemp.hpp"
#include "qmlpagelayoutdemo_dyncinit_priv.hxx"
...
// ////////////////////////////////////////////////////////////////////////////
class QMLPAGELAYOUTDEMO_HIDDEN MemCntrRec {
...
class QMLPAGELAYOUTDEMO_HIDDEN MemCntrConstStrCmp {
public:
...
class QMLPAGELAYOUTDEMO_HIDDEN MemCntrRecDescr {
...
// action cb object
// ////////////////////////////////////////////////////////////////////////////
class QMLPAGELAYOUTDEMO_HIDDEN MemCntrRecAction {
...
typedef qmlPagelayoutDemo::Deque<MemCntrRec>        MemCntrRecQueue;
typedef qmlPagelayoutDemo::Deque<MemCntrRecAction>  MemCntrRecActionQueue;

class QMLPAGELAYOUTDEMO_HIDDEN MemCntrRecThread {
private:
...
void   MemCntrRecThread :: procRec_nw( const char *fp, const char *type, int cntr )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){  qmlpagelayoutdemo_objcast(  MemCntrRecThread *, ctxt )->procRec_nw_Act_pr(); }, this
           ));
    m_sem.notify();
}
...
void   MemCntrRecThread :: saveToFile( const char *fn )
{
    ...
    m_rec_act_queue_ts.push( MemCntrRecAction(
                  [](void*ctxt){
                        SaveToFileContxt *st = qmlpagelayoutdemo_objcast( SaveToFileContxt *, ctxt );
...
static void  gInitRes( )
{
    ...
    if ( ! *qmlpagelayoutdemo_ptrcast( volatile bool *, &is_init ) ) {
        is_init = true;
    }
}
...
int  MemCntr ::  incrNewCntr( int cntr, const char *file, const char *func, int ln, const char *type )
{
    ...
    if ( gIsEnableMemTrace() ) {
        ...
        QMLPAGELAYOUTDEMO_UNUSED(func);
        QMLPAGELAYOUTDEMO_UNUSED(ln);
    }
#else
    QMLPAGELAYOUTDEMO_UNUSED(file); QMLPAGELAYOUTDEMO_UNUSED(func);
    QMLPAGELAYOUTDEMO_UNUSED(ln);   QMLPAGELAYOUTDEMO_UNUSED(type);
#endif
    return ct;
}
...
void*  MemCntr :: _assertNonNullPtr( void *p, bool is_alloc )
{
    if ( p == nullptr ) {
        if ( is_alloc ) {
            qmlpagelayoutdemo_fatal("allocated memory, but return nullptr!");
        } else {
            qmlpagelayoutdemo_fatal("free memory, pass nullptr!");
        }
    }
    return p;
}
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlpagelayoutdemo_ptrcast( char *, std::malloc( size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
...
{
    if ( size < 1 ) { size = 1; }
    char *p = qmlpagelayoutdemo_ptrcast( char *, std::realloc( old, size_t( size )));
    if ( p == nullptr ) {
        std::abort();
    }
    return p;
